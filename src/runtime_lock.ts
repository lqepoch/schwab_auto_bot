...
export function acquireRuntimeLock(path: string, runId: string, pid = process.pid): RuntimeLock {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      return {
        release: () => {
          ...
          try {
            ...
            if (current?.runId === runId && current.pid === pid) unlinkSync(path);
          } catch (error) {
    ...
    try {
      renameSync(path, `${path}.stale.${current.pid}.${current.runId}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
  }
...
function readLock(path: string): RuntimeLockRecord | null {
  try {
    ...
    if (value.schemaVersion !== 1 || !Number.isInteger(value.pid) || value.pid <= 0 || typeof value.runId !== "string" || !value.runId) {
      return null;
    }
