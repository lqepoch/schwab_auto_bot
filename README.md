...
整体刷新默认每个候选订单间隔 1 秒、每轮完成后间隔 10 秒。每一轮开始都会读取一次完整订单列表；该快照同时供本轮所有候选筛选与原生 Replace 使用，不会在每次 Replace 前发起 `/orders/{orderId}` 的额外 GET。订单列表与账户、持仓属于不同 Schwab 端点，不能合成单一 HTTP 请求；程序会复用这份完整快照，避免同轮重复读取。没有可用 ID 或刷新失败时，该候选在本轮失败，下一轮从新的完整快照重新判断。
...
每个 0DTE 垂直组合都有独立、持久化的卖出评估 worker，不会被其他组合的卖出动作阻塞。每次 worker 唤醒都会先用新鲜的持仓快照核对该组合可平数量；账户级持仓读取最多在 1 秒内合并一次，不会改变各组的独立触发与写入决策。每张完整买入垂直订单以其自身 Schwab 成交时刻开始计时；满 30 秒时会直接唤醒对应 worker。热切换或重启时，已保存模板和最近成交订单也会立即恢复该 worker。后续新成交不会延后已经成熟的早期成交。某组合可平仓仓位达到 5 张时，立即将该组合的卖出目标提高到全部可平仓仓位。

卖单保持每张数量 1、价格 0.99。正常刷新间隔为 8 秒，卖单目标数量等于该组合当前可平仓仓位中已经成熟的单张数量。程序会保存最近的组合模板至 `.state/exit-templates.json`，因此热切换后仍能为已有仓位恢复独立卖出 worker。

如果 Schwab 对新的买入 `Preview` 明确返回资金、现金或购买力不足，程序会立即对对应组合刷新持仓并启动流动性卖出：跳过 30 秒等待、将卖单目标提升至该组合全部可平仓仓位，并以 5 秒间隔强制刷新两轮，随后恢复正常 8 秒节奏。该触发及每轮结果会写入执行审计日志中的 `exit.liquidity-*` 与 `exit.gate` 事件。
