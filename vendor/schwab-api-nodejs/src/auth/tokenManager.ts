...
export class TokenManager {
  ...
  async refreshAccessToken(refreshToken: string): Promise<PersistedToken> {
    ...
    const pending = this.requestToken(body)
      .then((token) => {
   ...
   */
  private async requestToken(body: URLSearchParams): Promise<PersistedToken> {
    const start = performance.now();
    ...
    if (!response.ok) {
      ...
      if (analysis.isInvalidGrant) {
        this.logger.error('检测到 OAuth invalid_grant，刷新令牌可能已失效或被重复使用', {
          description: analysis.description,
        });
        this.emitInvalidGrant({ description: analysis.description, body: analysis.sanitizedBody });
      }
      throw new Error(`Schwab token request failed: ${response.status} ${response.statusText} - ${text}`);
    }
    let payload: unknown;
    try {
  ...
  private analyzeOAuthError(rawBody: string): {
    ...
    const sanitizedBody = redactSensitive(parsed ?? rawBody);
